    ...
    <div className='overflow-hidden shadow-lg border-[rgba(0,0,0,0.1)] rounded-md productItem border'>
      ...
      <div className='p-3 py-5 info'>
        <h6 className='text-[13px] !font-[400]'>
          <Link href='/product/4874' className='transition-all link'>Soylent Green</Link>
        </h6>
        <h3 className='text-[13px] title mt-1 mb-1 font-[500] text-[#000]'>
          <Link href='/product/4874' className='transition-all link'>Sirii Georgette Pink Color Saree with Blouse piece</Link>
        </h3>
      ...
      {/* Example: Show modal when clicked */}
      {openProductDetailsModal && (
        <div className="fixed top-0 left-0 w-full h-full bg-black/50 flex items-center justify-center z-50">
          <div className="bg-white p-6 rounded-lg shadow-lg w-[400px]">
            <h2 className="text-lg font-bold mb-4">Product Details</h2>
            <p>This is the product details modal.</p>
            <Button variant="contained" color="primary" onClick={() => setOpenProductDetailsModal(false)}>
              Close
            </Button>
          </div>
        </div>
